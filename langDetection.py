...
MaxWords = 10000    #maximum words that uses in model
MaxLen = 400        
...
trainDf = Tkns.texts_to_sequences(list(trainDf['sentence']))
# padding seuqnces
trainDf = tf.keras.preprocessing.sequence.pad_sequences(trainDf,
                                                       maxlen=MaxLen,
                                                       padding='pre')
#%% Train Test Split
XTrain,XTest,YTrain,YTest = train_test_split(trainDf,target,test_size=0.2,
                                                            random_state=42)
#%% creat model
WVD = 100
model = tf.keras.models.Sequential([tf.keras.layers.Embedding(input_dim=VocSize,
                                                             output_dim=WVD,
                                                             input_length=MaxLen),
...
epochs = np.linspace(1,NEpochs,num=NEpochs)
